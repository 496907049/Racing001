...
public class ISchedulePresentImpl implements IBasePresent {

    private Context context;
    private IBaseView baseView;

    public ISchedulePresentImpl(IBaseView baseView, Context context) {
        ...
        HttpClientUtil.getInstance(context).get(url,new AsyncHttpResponseHandler() {
            ...
            public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
                ...
                try {
                    if (string != null) {
                        Gson gson = new Gson();
                        List<Schedule> beanList = (List<Schedule>) gson.fromJson(string, new TypeToken<List<Schedule>>() {}.getType());
